...
public class MemberAuth extends HttpServlet {
    ...
    protected void service(HttpServletRequest req, HttpServletResponse resp)
        ...
        //회원인증을 위한 메서드를 호출한다. 일치하는 정보가 있다면
        //회원 레코드를 DTO에 저장한 후 반환한다.
        ...
        if (memberDTO != null && memberDTO.getName() != null) {
            message = memberDTO.getName() + " 회원님 방가방가 ^^";
        } else {
            if (admin_id != null && admin_id.equals(id)) {
                message = admin_id + "는 최고관리자 입니다";
            } else {
                message = "귀하는 회원이 아닙니다";
            }
        }

        req.setAttribute("authMessage", message);
